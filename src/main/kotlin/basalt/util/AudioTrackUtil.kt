...
class AudioTrackUtil internal constructor(private val server: BasaltServer) {
    ...
    fun fromAudioTrack(track: AudioTrack): String {
        try {
            ...
            server.playerManager.encodeTrack(MessageOutput(out), track)
    ...
    fun toAudioTrack(data: String): AudioTrack {
        try {
            return server.playerManager.decodeTrack(MessageInput(ByteArrayInputStream(Base64.decode(data)))).decodedTrack
        } catch (err: IOException) {
